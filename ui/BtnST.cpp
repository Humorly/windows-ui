...
void CButtonST::DrawItem(LPDRAWITEMSTRUCT lpDIS)
{
  ...
  else
  {
	...
	if (m_bDrawTransparent == TRUE)
	{
		...
		CBrush brrr;
		brrr.CreateSolidBrush(bgColor);
		pDC->SelectObject(brrr);
...
*/
  ...
  if (sTitle.IsEmpty() == FALSE)
  {
	...
	if ((m_MouseOnButton == TRUE) || (bIsPressed))
	{
		pDC->SetTextColor(OWNER_COLOR_GRAY_TEXT);
...
{
	m_crInactiveBg = OWNER_COLOR_BACKGROUND_BTN/*::GetSysColor(COLOR_BTNFACE)*/;
...
{
	m_crInactiveFg = OWNER_COLOR_HIGHLIGHT_BTN/*::GetSysColor(COLOR_BTNTEXT)*/;
	if (bRepaint == TRUE) Invalidate();
...
{
	m_crActiveBg = OWNER_COLOR_HIGHLIGHT_BTN/*::GetSysColor(COLOR_BTNFACE)*/;
	if (bRepaint == TRUE) Invalidate();
...
{
	m_crActiveFg = OWNER_COLOR_BACKGROUND_BTN/*::GetSysColor(COLOR_BTNTEXT)*/;
	if (bRepaint == TRUE) Invalidate();
